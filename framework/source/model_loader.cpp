...
namespace model_loader {
    ...
    std::vector<glm::fvec3> generate_tangents(tinyobj::mesh_t const& model) {
        ...
        for (unsigned i = 0; i < model.indices.size() / 3; i++) {
            ...
            glm::fvec3 pos2 = positions[indices[2]];

            glm::fvec2 uv0 = texcoords[indices[0]];
            glm::fvec2 uv1 = texcoords[indices[1]];
            glm::fvec2 uv2 = texcoords[indices[2]];

            glm::fvec3 deltaPos1 = pos1 - pos0;
            glm::fvec3 deltaPos2 = pos2 - pos0;

            glm::fvec2 deltaUV1 = uv1 - uv0;
            glm::fvec2 deltaUV2 = uv2 - uv0;

            float div = 1.0f / (deltaUV1.x * deltaUV2.y - deltaUV2.x * deltaUV1.y);

            glm::fvec3 tangent;

            tangent.x = div * (deltaUV2.y * deltaPos1.x - deltaUV1.y * deltaPos2.x);
        ...
        for (unsigned i = 0; i < tangents.size(); ++i) {
            ...
            std::cout<<tangents[i].x<<", "<<tangents[i].y<<","<<tangents[i].z<<"\n";
        }
